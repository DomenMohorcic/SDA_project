...
```{r}
knitr::kable(head(roller_coasters_raw))
```
...
```
Lets have a look at the categorical variables first. We will skip the Name, Park,
...
for the rest, look at the summary below:
...
From the density plot and boxplot we can observe that wooden coaster are on average faster than the steel ones. This will also be one of the hypothesis tests later on to confirm our observations.
...
If we meet those criteria, we can infer about the population based on the analysis we do on the sample. We firstly assume that all the instances are independent. We can also see that there are more than enough instances:
...
We can alsocalculate 95% confidence intervals:
...
Our goal is to make a linear regression model for prediction of coasters Speed attribute.
...
From the pairplot below, we can observe that there are some linear or non linear relationships between length, height, speed, and numinversions:
...
From the above linear regression analysis and plots we are able to see that there is indeed a linearity (as p values show). But there are some influential outliers! Not to forget, the residuals also don't have a quite constant varibility. Although the model has a high R^2 we need to be careful when using this model since it does not completely meet the requirements of linear regression analysis.
...
Our confidence intervals are as followed:
intercept ->  [28.90, 38,49]
length ->     [0.010, 0.017]
height ->     [1.144, 1.301]
steel ->      [-10.0, -1.97]
...
We can see from both p-values that weight is not normally distributed neither for
...
We want to know if there is a difference between the males in east and west coast.
...
value, which is only `r summary(fit_wh)$r.squared`. That means that our model is
...
and a season. Both variables influence the weight of seagulls. We can try and build
...
We can see that our new model is much better with adjusted R-squared of `r summary(fit_wh)$adj.r.squared`.
The formula for determining weight of a seagull is then
$$WEIGHT = 200.07681 + 1.96847 * HEIGHT - 19.64618 * WINTER + 13.46615 * MALE$$
where HEIGHT is in centimeters (cm), and WINTER and MALE are 0 or 1 depending on a season
and a seagull. WEIGHT is in grams (g).

We can also construct confidence intervals for every predictor variable. The confidence
intervals are thus $`r summary(fit_wh)$coefficient[1,1]`\pm`r qt(0.975, summary(fit_wh)$df[2])*summary(fit_wh)$coefficient[1,2]`$
for Intercept, $`r summary(fit_wh)$coefficient[2,1]`\pm`r qt(0.975, summary(fit_wh)$df[2])*summary(fit_wh)$coefficient[2,2]`$
for height, $`r summary(fit_wh)$coefficient[3,1]`\pm`r qt(0.975, summary(fit_wh)$df[2])*summary(fit_wh)$coefficient[3,2]`$
for if season is winter, and $`r summary(fit_wh)$coefficient[4,1]`\pm`r qt(0.975, summary(fit_wh)$df[2])*summary(fit_wh)$coefficient[4,2]`$ for if it is a male.
...
```{r}
200.07681 + 1.96847 * 40 - 19.64618 * 0 + 13.46615 * 0
```